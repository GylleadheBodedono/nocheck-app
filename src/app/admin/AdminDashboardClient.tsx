...
type AdminDashboardClientProps = {
  ...
  initialAllStores: { id: number; name: string }[]
  initialIgnoreTimeRestrictions: boolean
  initialBypassStoreIds: number[] | 'all'
}

/**
 * Dashboard admin (`/admin`) — Client component.
 * Receives server-fetched data as props and handles realtime updates,
 * notification permissions, and time restriction toggles.
 */
export default function AdminDashboardClient({
  initialStats,
  initialPreview,
  initialAllStores,
  initialIgnoreTimeRestrictions,
  initialBypassStoreIds,
}: AdminDashboardClientProps) {
  const [stats, setStats] = useState<Stats>(initialStats)
  const [preview, setPreview] = useState<PreviewData>(initialPreview)
  const [ignoreTimeRestrictions, setIgnoreTimeRestrictions] = useState(initialIgnoreTimeRestrictions)
  const [togglingTime, setTogglingTime] = useState(false)
  const [bypassStoreIds, setBypassStoreIds] = useState<number[] | 'all'>(initialBypassStoreIds)
  const [allStores] = useState<{ id: number; name: string }[]>(initialAllStores)
  ...
  // Realtime refetch on data changes
  useEffect(() => {
    if (refreshKey > 0 && navigator.onLine && isSupabaseConfigured && supabase) {
