...
/**
 * Client component for the settings page (`/admin/configuracoes`).
 * Handles email template editing, preview, save, test email, and restore defaults.
 */
export default function ConfiguracoesPageClient({
  ...
  // Template state
  const [emailSubject, setEmailSubject] = useState(initialEmailSubject ?? DEFAULT_ACTION_PLAN_EMAIL_SUBJECT)
  const [emailTemplate, setEmailTemplate] = useState(initialEmailTemplate ?? DEFAULT_ACTION_PLAN_EMAIL_HTML)
  const [showPreview, setShowPreview] = useState(false)
  const [showVariables, setShowVariables] = useState(false)
  const [copiedVar, setCopiedVar] = useState<string | null>(null)

  // Reload settings when realtime refresh triggers
  useEffect(() => {
