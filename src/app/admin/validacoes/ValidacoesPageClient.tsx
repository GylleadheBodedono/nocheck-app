...
type Props = {
  initialValidations: CrossValidation[]
  initialStores: Store[]
  initialSectors: { id: number; name: string; store_id: number }[]
  initialExpirationMinutes: number
}

/**
 * Client component for the validacoes page (`/admin/validacoes`).
 * Receives server-fetched data as props and handles all interactivity:
 * filters, mutations (delete, save expiration), Teams export, realtime refresh.
 */
export default function ValidacoesPageClient({
  initialValidations,
  initialStores,
  initialSectors,
  initialExpirationMinutes,
}: Props) {
  const [validations, setValidations] = useState<CrossValidation[]>(initialValidations)
  ...
  const [sectors, setSectors] = useState<{ id: number; name: string; store_id: number }[]>(initialSectors)
  const [exporting, setExporting] = useState(false)
  const [exportMessage, setExportMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null)
  const [deleting, setDeleting] = useState<number | null>(null)
  const [expirationMinutes, setExpirationMinutes] = useState<number>(initialExpirationMinutes)
  ...
  const refetchData = async () => {
    try {
      ...
      logError('[Validacoes] Erro ao atualizar dados', { error: err instanceof Error ? err.message : String(err) })
    }
  }
  ...
  const handleDelete = async (validationId: number, linkedId?: number | null) => {
    if (!confirm('Tem certeza que deseja excluir esta validacao?')) return
    ...
    try {
      ...
      alert('Erro ao excluir validacao')
    }
    setDeleting(null)
  }
  ...
  const groupValidations = (validations: CrossValidation[]): GroupedValidation[] => {
    ...
    for (const validation of validations) {
      ...
      // Se e uma validacao primaria com link
      ...
      // Se e uma validacao secundaria (nao primaria)
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted mt-1">
                ...
                Quando os valores ou notas divergem, o sistema tenta vincular notas &quot;irmas&quot; baseado em:
                mesma loja, mesmo setor, horario proximo (ate 30 min) e prefixo similar.
                Notas sem par apos <strong className="text-purple-500">{formatExpirationLabel(expirationMinutes)}</strong> sao marcadas como <strong className="text-purple-500">expiradas</strong>.
              </p>
            </div>
          </div>
        </div>
