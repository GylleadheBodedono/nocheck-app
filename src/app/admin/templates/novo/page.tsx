                              ...
                              {(fieldListeners) => (<>
                                <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3">
                                  ...
                                  <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
                                    <label className="flex items-center gap-0.5 sm:gap-1 text-[10px] sm:text-xs text-secondary" title="Obrigatório">
                                  ...
                                  <div className="px-2 pb-2 sm:px-3 sm:pb-3 pt-2 border-t border-subtle space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                      ...
                                      <div>
                                        ...
                                        <input type="text" value={field.help_text} onChange={(e) => updateField(field.id, { help_text: e.target.value })} className="input text-sm" placeholder="Instruções para o usuário..." />
                                      </div>
                                    </div>
                                    {sections.length > 0 && (
                                      <div>
                                    ...
                                    {field.field_type === 'number' && (
                                      <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                          {[{ value: 'monetario', label: 'Monetário (R$)' }, { value: 'quantidade', label: 'Quantidade (un)' }, { value: 'decimal', label: 'Decimal' }, { value: 'porcentagem', label: 'Porcentagem (%)' }].map(st => (
                                        ...
                                        <div className="space-y-2">
                                          {getOptionsItems(field.options).map((opt: string, optIdx: number) => (
                                            <div key={optIdx} className="flex items-center gap-2">
                                              ...
                                              <input type="text" value={opt} onChange={(e) => { const newOpts = [...getOptionsItems(field.options)]; newOpts[optIdx] = e.target.value; updateField(field.id, { options: getFieldIcon(field) ? { items: newOpts, icon: getFieldIcon(field) } : newOpts }) }} placeholder={`Opção ${optIdx + 1}`} className="input text-sm flex-1" />
                                    ...
                                    {field.field_type === 'yes_no' && (
                                      <div className="space-y-2">
                                        ...
                                        {(field.options as { allowPhoto?: boolean } | null)?.allowPhoto && (
                                          <Select value={(field.options as { photoRequired?: boolean } | null)?.photoRequired ? 'required' : 'optional'} onChange={(v) => updateField(field.id, { options: { ...((field.options as Record<string, unknown>) || {}), photoRequired: v === 'required' } })} className="text-sm" options={[{ value: 'optional', label: 'Foto opcional' }, { value: 'required', label: 'Foto obrigatória' }]} />
                                        )}
                                        ...
                                        <div className="p-3 bg-red-500/5 border border-red-500/20 rounded-lg space-y-2">
                                          <p className="text-xs font-medium text-red-400">Quando resposta for &quot;Não&quot;:</p>
                                          <label className="flex items-center gap-2 text-sm text-secondary cursor-pointer">
                      ...
                      {(fieldListeners) => (<>
                        <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-4">
                          ...
                          <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                            <label className="flex items-center gap-1 sm:gap-2 text-[10px] sm:text-sm text-secondary" title="Obrigatório">
                              ...
                              <span className="hidden sm:inline">Obrigatório</span><span className="sm:hidden">*</span>
                            </label>
