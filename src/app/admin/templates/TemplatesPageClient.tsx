...
/**
 * Componente cliente da página de listagem de templates (`/admin/templates`).
 ...
 */
export default function TemplatesPageClient({ initialTemplates, initialFavoriteIds, currentUserId }: Props) {
  ...
  useEffect(() => {
    if (refreshKey > 0 && navigator.onLine) refetchTemplates()
  ...
  const refetchTemplates = async () => {
    ...
    if (!error && data) setTemplates(data as TemplateWithDetails[])
  }
  ...
  const getFieldTypeLabel = (type: string) => {
    const labels: Record<string, string> = {
      ...
      datetime: 'Data/Hora',
      checkbox_multiple: 'Multipla Escolha',
      gps: 'GPS',
