                              ...
                              <div key={field.id} className={`border rounded-xl transition-all ${editingField === field.id ? 'border-primary bg-surface-hover' : 'border-subtle bg-surface'}`}>
                                ...
                                {editingField === field.id && (
                                  <div className="px-2 pb-2 sm:px-3 sm:pb-3 pt-2 border-t border-subtle space-y-3">
                                    ...
                                    {(field.field_type === 'dropdown' || field.field_type === 'checkbox_multiple') && (<div><label className="block text-xs text-muted mb-1">Opcoes (uma por linha)</label><textarea value={Array.isArray(field.options) ? field.options.join('\n') : ''} onChange={(e) => updateField(field.id, { options: e.target.value.split('\n').filter((o: string) => o.trim()) })} rows={4} className="input text-sm resize-none" placeholder="Opcao 1&#10;Opcao 2&#10;Opcao 3" /></div>)}
