...
export default function PlatformDashboard() {
  ...
  // Rankings
  const [mostLoyal, setMostLoyal] = useState<RankingItem[]>([])
  const [oldest, setOldest] = useState<RankingItem[]>([])
  const [mostDelayed, setMostDelayed] = useState<RankingItem[]>([])
  const [mostActive, setMostActive] = useState<RankingItem[]>([])
  const [leastActive, setLeastActive] = useState<RankingItem[]>([])
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setOrgs(all.sort((a: { created_at: string }, b: { created_at: string }) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 10).map((o: OrgRow) => ({ ...o, member_count: mm[o.id] || 0, store_count: sm[o.id] || 0 })))
        ...
        if (completedChecklists) {
          ...
          setMostLoyal(Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([id, count]) => ({
            org_id: id, org_name: (orgMap.get(id) as string) || id, value: `${count} checklists`
          })))
        }

        // 2. Oldest clients
        setOldest(all.sort((a: { created_at: string }, b: { created_at: string }) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()).slice(0, 5).map((o: { id: string; name: string; created_at: string }) => ({
        ...
        if (pendingChecklists) {
          ...
          setMostDelayed(Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([id, count]) => ({
        ...
        // Initialize all active orgs with 0
        ...
        setMostActive(sortedActivity.slice(0, 5).map(([id, count]) => ({
          org_id: id, org_name: (orgMap.get(id) as string) || id, value: `${count} acoes`
        })))
        setLeastActive(sortedActivity.reverse().slice(0, 5).map(([id, count]) => ({
          org_id: id, org_name: (orgMap.get(id) as string) || id, value: `${count} acoes`
        })))
