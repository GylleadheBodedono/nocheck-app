    ...
    caches.open(STATIC_CACHE)
      .then(cache => {
        ...
        return cache.addAll([
          ...
          console.log('[SW v5] Some static assets failed to cache:', err)
        })
      })
      .then(() => {
...
async function cacheFirst(request) {
  ...
  try {
    ...
    console.log('[SW v5] Fetch failed:', request.url)
...
async function networkFirstForNavigation(request) {
  ...
  try {
    ...
    console.log('[SW v5] Network failed for navigation:', url.pathname)
    ...
    for (const alt of alternatives) {
      ...
      if (cached) {
        console.log('[SW v5] Serving alternative:', alt)
        return cached
      }
    }
...
async function cacheAssetsFromHtml(response, origin) {
  try {
      ...
      try {
        ...
        if (!existing) {
          ...
          if (assetResponse.ok) {
            ...
            console.log('[SW v5] Cached asset:', url)
          }
        }
      } catch {
    ...
    console.log('[SW v5] Error caching assets from HTML:', error)
  }
}
...
async function precacheApp() {
  console.log('[SW v5] Starting FULL app precache...')
  ...
  for (const asset of staticAssets) {
    try {
      ...
      if (response.ok) {
        ...
        totalCached++
        console.log('[SW v5] Cached static:', asset)
      }
    } catch {
      console.log('[SW v5] Failed static:', asset)
    }
  }
  ...
  for (const pageUrl of pages) {
    try {
      ...
      if (response.ok) {
        ...
        totalCached++
        console.log('[SW v5] Cached page:', pageUrl)
        ...
        console.log('[SW v5] Found', assetUrls.size, 'assets in', pageUrl)
      ...
      console.log('[SW v5] Failed page:', pageUrl, err)
    }
  }

  console.log('[SW v5] Precache COMPLETE! Total items:', totalCached)

  // Lista todos os items no cache
  const cachedItems = await cache.keys()
  console.log('[SW v5] Cache now contains', cachedItems.length, 'items')
}
