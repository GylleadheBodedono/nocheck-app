// OpereCheck Service Worker v17.0.0
    ...
    caches.open(STATIC_CACHE)
      .then(cache => {
        ...
        return cache.addAll([
          ...
          console.log('[SW v17] Some static assets failed to cache:', err)
        })
      })
      .then(() => {
        console.log('[SW v17] Install complete, waiting for activation...')
    ...
    caches.keys()
      .then(async (keys) => {
        ...
        if (oldAppCaches.length > 0) {
          console.log('[SW v17] Migrating entries from old app caches:', oldAppCaches)
...
self.addEventListener('fetch', (event) => {
  ...
  if (request.mode === 'navigate') {
    console.log('[SW v17] Navigate:', url.pathname, url.search)
    ...
    if (url.search.includes('error=') || url.search.includes('code=') || url.search.includes('token') || url.pathname.startsWith('/auth') || url.pathname === '/') {
      console.log('[SW v17] BYPASS (not cached):', url.pathname)
      return // Deixa o browser lidar normalmente
    }
    console.log('[SW v17] networkFirst:', url.pathname)
...
async function cacheFirst(request) {
  ...
  try {
    ...
    console.log('[SW v17] Fetch failed:', request.url)
...
async function networkFirstForNavigation(request) {
  ...
  try {
    ...
    console.log('[SW v17] Network failed for navigation:', url.pathname)

    // Tenta o cache
    const cachedResponse = await caches.match(request)
    if (cachedResponse) {
      console.log('[SW v17] Serving from cache:', url.pathname)
      return cachedResponse
    }
    ...
    for (const alt of alternatives) {
      ...
      if (cached) {
        console.log('[SW v17] Serving alternative:', alt)
        return cached
      }
    }
...
async function cacheAssetsFromHtml(response, origin) {
  try {
      ...
      try {
        ...
        if (!existing) {
          ...
          if (assetResponse.ok) {
            ...
            console.log('[SW v17] Cached asset:', url)
          }
        }
      } catch {
    ...
    console.log('[SW v17] Error caching assets from HTML:', error)
  }
}
...
self.addEventListener('message', async (event) => {
  ...
  switch (type) {
    ...
    case 'CLEAR_CACHE':
      ...
      console.log('[SW v17] All caches cleared')
...
async function precacheApp() {
  console.log('[SW v17] Starting FULL app precache...')
  ...
  for (const asset of staticAssets) {
    try {
      ...
      if (response.ok) {
        ...
        totalCached++
        console.log('[SW v17] Cached static:', asset)
      }
    } catch {
      console.log('[SW v17] Failed static:', asset)
    }
  }
  ...
  for (const pageUrl of pages) {
    try {
      ...
      if (response.ok) {
        ...
        totalCached++
        console.log('[SW v17] Cached page:', pageUrl)
        ...
        console.log('[SW v17] Found', assetUrls.size, 'assets in', pageUrl)
      ...
      console.log('[SW v17] Failed page:', pageUrl, err)
    }
  }

  console.log('[SW v17] Precache COMPLETE! Total items:', totalCached)

  // Lista todos os items no cache
  const cachedItems = await cache.keys()
  console.log('[SW v17] Cache now contains', cachedItems.length, 'items')
}
...
self.addEventListener('sync', (event) => {
  if (event.tag === 'sync-checklists') {
    console.log('[SW v17] Background sync triggered')
    event.waitUntil(
...
console.log('[SW v17] Service Worker loaded')
